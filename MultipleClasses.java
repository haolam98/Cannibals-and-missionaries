...
Work's Explanation:
	...
	- For this problem, we choose to apply Breadth-first search (BFS) algorithm to run for the solution.
		...
		+ Repeat all steps above until reaching a child_state that equals to goal_state. Then, finally, return as the solution.
	In the end, the output solution will be one of the most efficient solutions. The algorithm will stop and return the result as soon as there's one reaching the goal state.
	...
	- The output result will display the solution and states to get to the goal state (0,0,R,3,3) from the initial state (3,3,L,0,0).
		+ Output expected to be when running the code as below:
